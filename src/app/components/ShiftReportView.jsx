...
export default function ShiftReportView({ submissions = [], users = [] }) {
  ...
  const [selectedShift, setSelectedShift] = useState('t1'); // 't1' (1pm), 't2' (7pm), 't3' (12am), 'all' (completo)

  // ── DEFAULT INITIAL STATE IS SET TO 0 BY USER REQUEST ──
  ...
  // Dynamic Rooms List (Supports Up to 14 rooms in 2 columns: 7 left, 7 right!)
  ...
  // Pre-load official social network logos
  useEffect(() => {
    const cache = {};
    let loadedCounter = 0;
  ...
  // Auto-calculate stats from system database based on selected turn
  ...
  useEffect(() => {
    ...
    function drawFullReport() {
      ...
      // ── CARD 2: REPORTES DE SALAS EXTERNAS (SUPPORTS UP TO 14 ROOMS IN 2 COLUMNS: 7 LEFT, 7 RIGHT) ──
      ...
      // Compact Inline Summary for Activated, Reporting & Pending
      ctx.fillStyle = '#0f172a';
      ...
      // Render 2 Columns for Custom External Rooms (7 Left, 7 Right = Up to 14 rooms!)
      ...
      // ── CARD 3: ANÁLISIS RÁPIDO (WITH OFFICIAL LOGOS) ──
      ...
      nets.forEach((n, idx) => {
        ...
        // Draw Official Social Network Logo Image
      ...
      // ── CARD 4: SUGERENCIAS Y RECOMENDACIONES (Bottom-Right) ──
