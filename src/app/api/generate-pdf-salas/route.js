...
export async function GET(request) {
  try {
    ...
    // Helper to parse Firestore response
    ...
    while (true) {
      ...
      if (!response.ok) throw new Error(`Submissions fetch error! status: ${response.status}`);
    ...
    return new NextResponse(pdfBuffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Reporte_Salas.pdf"`
      }
    });
